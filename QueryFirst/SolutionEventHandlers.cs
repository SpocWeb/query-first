...
"
        ...
        private async Task myDocumentEvents_DocumentOpenedAsync(Document Document)
        {
            if (!connecting)
            {
                // we need to let the window initialise, otherwise intellisense is broken
                _ = Task.Run(async () => await connectEditorWindow2DBAsync(Document));
                await Task.Delay(0);
            }
        }
        private bool connecting = false;
        private async Task connectEditorWindow2DBAsync(Document Document)
        {
            try
            {
        ...
        async Task myDocumentEvents_DocumentSavedAsync(Document Document)
        {
            await ThreadHelper.JoinableTaskFactory.SwitchToMainThreadAsync();
